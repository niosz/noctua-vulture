...
export class NoctuaVulture implements IProxy {
  ca!: ca;
  ...
  _onWebSocketServerConnect(
    ...
    if (upgradeReq.url == "" || /^\//.test(upgradeReq.url!)) {
      const hostPort = NoctuaVulture.parseHostAndPort(upgradeReq);
  ...
  _onHttpServerRequest(
    ...
    const hostPort = NoctuaVulture.parseHostAndPort(
      ...
      return self._onResponse(ctx, (err) => {
        ...
        return self._onResponseHeaders(ctx, (err) => {
          ...
          ctx.proxyToClientResponse.writeHead(
            ...
            NoctuaVulture.filterAndCanonizeHeaders(servToProxyResp.headers)
          );
          // @ts-ignore
  ...
  static parseHostAndPort(req: http.IncomingMessage, defaultPort?: number) {
    ...
    if (m) {
      req.url = m[2] || "/";
      return NoctuaVulture.parseHost(m[1], defaultPort);
    } else if (req.headers.host) {
      return NoctuaVulture.parseHost(req.headers.host, defaultPort);
    } else {
      return null;
    }
  }
